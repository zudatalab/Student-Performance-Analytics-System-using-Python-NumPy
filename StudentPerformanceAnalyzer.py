...
# Pass/Fail detection
passing_marks = np.all(marks >= 40, axis=1)
pass_students = np.sum(passing_marks)
...
grades[(marks >= 55) & (marks < 70)] = 'D'
grades[(marks >= 40) & (marks < 55)] = 'E'
grades[marks < 40] = 'F'
#Outputs:-
...
for i, status in enumerate(passing_marks, 1):
    print(f" Roll No {i:02d}: {'Pass' if status else 'Fail'}")
